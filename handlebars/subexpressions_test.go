...
var subexpressionsTests = []Test{
	...
	},
	{
		...
		nil,
		map[string]interface{}{
			"blog": func(options *mario.Options) string {
	...
	},
	{
		...
		nil,
		map[string]interface{}{
			"input": func(options *mario.Options) mario.SafeString {
				return mario.SafeString(`<input aria-label="` + options.HashStr("aria-label") + `" placeholder="` + options.HashStr("placeholder") + `" />`)
			},
			"t": func(param string) mario.SafeString {
				return mario.SafeString(param)
			},
		},
		nil,
